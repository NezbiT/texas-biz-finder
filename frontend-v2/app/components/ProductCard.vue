...
// t() viene de @nuxtjs/i18n (auto-importado), no del useI18n casero de la v1
const { t } = useI18n()
</script>
...
<template>
  ...
  >
    <!-- Tailwind v4: bg-gradient-to-* pasó a llamarse bg-linear-to-* -->
    <div
      class="pointer-events-none absolute inset-0 bg-linear-to-br opacity-80"
      :class="product.accent"
    />
    <div class="relative">
      <div class="mb-4 flex items-start justify-between gap-3">
        <div
          class="flex h-11 w-11 items-center justify-center rounded-xl bg-brand-navy text-white shadow-xs dark:bg-white/10"
        >
      ...
      <div class="mt-5">
        ...
        >
          {{ t(product.ctaKey) }}
          <span aria-hidden="true">{{ product.external && product.sameOrigin === false ? '↗' : '→' }}</span>
        </ProductLink>
      </div>
    </div>
  </article>
